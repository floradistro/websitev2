...
export class VisualAnalyzer {
  ...
  async analyzeWebsite(
    ...
    const onProgress = options?.onProgress || ((status: string) => {});
    ...
    try {
              ...
              try {
                ...
                if (isVisible) {
                  ...
                  if (
                    ...
                    !newBodyText.includes("age") &&
                    !newBodyText.includes("verify")
                  ) {
                    dismissed = true;
                    break;
                  }
      ...
      const analysis = await page.evaluate(() => {
        ...
        const sections = document.querySelectorAll(
          'section, div[class*="section"], main > div, body > div > div',
        );
        sections.forEach((section, i) => {
          if (section.clientHeight > 200) {
            ...
            pageStructure.push(
              `Section ${i + 1}: ${title} (${hasImages} images, ${hasButtons} buttons)`,
            );
          }
        });
      ...
      if (metadata.width && metadata.width > targetWidth) {
        ...
        onProgress(
          `✅ Optimized: ${newMetadata.width}x${newMetadata.height}px (${sizeKB.toFixed(1)}KB)`,
        );
      }
...
🎨 Dominant Colors:
${analysis.insights.dominantColors
  .slice(0, 5)
  .map((c) => `• ${c}`)
  .join("\n")}
