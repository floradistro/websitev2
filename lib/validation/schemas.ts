...
export const ProductSchema = z.object({
  ...
  inventory_quantity: z.number().int().nonnegative('Inventory cannot be negative').optional().nullable(),
...
export const VendorSchema = z.object({
  ...
  slug: z.string().min(1).max(255).regex(/^[a-z0-9-]+$/, 'Slug must be lowercase alphanumeric with hyphens').trim(),
  email: z.string().email().toLowerCase().trim(),
  phone: z.string().max(20).optional().nullable(),
