...
export default function ShippingEstimator({
  ...
  useEffect(() => {}, [productId, quantity, productPrice, locationId]);
  ...
  return (
    <div className="border-t border-white/10 pt-6 space-y-4">
      ...
      {/* Error Message */}
      {error && (
        <div className="text-xs text-red-400 bg-red-500/10 border border-red-500/20 rounded-full px-4 py-2">
          {error}
        </div>
      )}
      ...
      {showResults && amountUntilFree > 0 && productPrice > 0 && (
        <div className="bg-gradient-to-r from-emerald-500/10 to-emerald-600/10 border border-emerald-500/20 rounded p-4">
      ...
      {showResults && (
        <div className="flex items-center gap-2 text-sm text-white/60 pt-4 border-t border-white/10 mt-4">
