...
export function CategoryModal({ isOpen, onClose, onSave, vendorId, category, categories }: CategoryModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      showNotification({
        type: 'error',
        title: 'Save Failed',
        message: error.response?.data?.error || `Failed to ${isEditing ? 'update' : 'create'} category`
      });
    } finally {
      setSaving(false);
    }
    ...
    >
      ...
      <div className={cn("p-4 rounded-lg mb-6", ds.colors.bg.elevated, ds.colors.border.default, "border")}>
        <div className="flex gap-3">
          <Folder className={cn("w-4 h-4 flex-shrink-0 mt-0.5", ds.colors.icon.blue)} strokeWidth={1.5} />
          <div>
