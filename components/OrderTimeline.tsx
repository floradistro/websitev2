...
export default function OrderTimeline({ 
  ...
  const getTimelineSteps = () => {
    const steps = [
      ...
      { id: 'shipped', label: 'Shipped', icon: Truck, tracking: trackingNumber },
      { id: 'completed', label: 'Delivered', icon: MapPin, date: dateCompleted },
    ];
  ...
  return (
    <div className="bg-[#2a2a2a] border border-white/10 p-6">
      <h3 className="text-sm uppercase tracking-[0.2em] text-white font-medium mb-8">Order Status</h3>
              ...
              <div className="flex-1 pt-2">
                <div className="flex items-center justify-between mb-1">
                  <h4 className={`text-sm uppercase tracking-wider font-medium ${
                    step.completed ? 'text-white' : step.current ? 'text-amber-400' : 'text-white/40'
                  }`}>
                    {step.label}
                  </h4>
                  {step.current && (
                ...
                {step.tracking && trackingNumber && (
                  <div className="mt-2">
                    <p className="text-[10px] uppercase tracking-wider text-white/40 mb-1">Tracking Number</p>
