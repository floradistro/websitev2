...
export default async function StorefrontCookiesPage({ searchParams }: { searchParams: Promise<{ vendor?: string; preview?: string }> }) {
  ...
  const sections = await getVendorSectionsWithInit(vendor.id, vendor.store_name || vendor.slug, 'cookies');
