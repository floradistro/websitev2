...
export function QuickAction({
  ...
  return (
    ...
    >
      <div
        className={`w-8 h-8 md:w-10 md:h-10 rounded-lg md:rounded-xl ${iconBg} flex items-center justify-center transition-all duration-300 group-hover:scale-110 border`}
      >
        <Icon
          size={16}
          className={`md:hidden ${variant === "highlight" ? "text-blue-400 group-hover:text-blue-300" : "text-white/60 group-hover:text-white"} transition-colors duration-300`}
          strokeWidth={1.5}
        />
        <Icon
          size={20}
          className={`hidden md:block ${variant === "highlight" ? "text-blue-400 group-hover:text-blue-300" : "text-white/60 group-hover:text-white"} transition-colors duration-300`}
          strokeWidth={1.5}
        />
      </div>
      ...
      {external && (
        <svg
          className="w-3 h-3 md:w-4 md:h-4 text-white/30 group-hover:text-white/60 transition-all duration-300 flex-shrink-0"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          strokeWidth={1.5}
