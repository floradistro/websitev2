...
export function Card({
  ...
  return (
    <div className={`${baseClass} ${paddingClass} ${hoverClass} rounded-xl md:rounded-2xl transition-all duration-300 ${className}`} style={style}>
      {children}
    </div>
  );
}
...
export function CardHeader({ title, subtitle, action, noPadding = false }: CardHeaderProps) {
  return (
    <div className={`flex items-center justify-between ${noPadding ? 'mb-4 md:mb-6' : 'p-4 md:p-6 border-b border-white/5'}`}>
      <div>
...
export function CardSection({ children, border = false, className = '' }: CardSectionProps) {
  return (
    <div className={`${border ? 'border-t border-white/5 pt-6' : ''} ${className}`}>
      {children}
    </div>
  );
}
