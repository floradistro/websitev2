...
export default function MenuEditorModal({
  ...
  // Debug logging
  console.log('[MenuEditorModal] Opened with:', {
    menu,
    availableCategories,
    availableCustomFields,
    menuConfigCategories: menu?.config_data?.categories
  });
  ...
  useEffect(() => {
    ...
    const fetchActualTiers = async () => {
      ...
      try {
        ...
        if (data.success && data.tiers) {
          ...
          categories.forEach(category => {
            ...
            if (actualTiers.length === 0) {
              console.warn(`No pricing tiers found for category: ${category}`);
              return;
            }
            ...
            updatedConfig[category] = {
              ...
              selected: existingSelection.filter(tier => actualTiers.includes(tier)).length > 0
        ...
        console.error('[MenuEditorModal] Error fetching category pricing tiers:', error);
      } finally {
  ...
  const [showSubCategories, setShowSubCategories] = useState(menu?.config_data?.showSubCategories || false);
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
          ...
          <div className="space-y-3">
            <div>
              <label className="block text-xs uppercase tracking-wider text-white/40 mb-2">Menu Name *</label>
              <input
                type="text"
                value={name}
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                >
                  <option value="grid" className="bg-black">Grid View</option>
                  <option value="list" className="bg-black">List View</option>
                </select>
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:ring-1 focus:ring-white/20 appearance-none cursor-pointer"
                >
                  <option value="single" className="bg-black">Single Category</option>
                  <option value="split" className="bg-black">Split Screen</option>
                </select>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between py-2 border-t border-white/5 mt-2 pt-4">
              <div>
                ...
                <div className="text-xs text-white/40">Display sub-categories as section headers</div>
              </div>
              <button
          ...
          {/* Categories */}
          <div>
            <h3 className="text-xs uppercase tracking-wider text-white/60 font-bold mb-3">Categories</h3>
            <CategorySelector
          ...
          {layoutStyle === 'split' && (
            <div className="space-y-4 p-4 bg-white/5 rounded-lg border border-white/10">
              <div className="flex items-center justify-between">
                <h3 className="text-xs uppercase tracking-wider text-white/60 font-bold">Split Screen Configuration</h3>
          ...
          {layoutStyle === 'single' && (
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <h3 className="text-xs uppercase tracking-wider text-white/60 font-bold">Product Information</h3>
                {availableCustomFields.length > 0 && (
                  <button
                    onClick={() => setHideAllFieldLabels(!hideAllFieldLabels)}
                    className={`text-xs px-2 py-1 rounded transition-all ${
              ...
              {customFields.length === 0 && (
                <p className="text-xs text-white/30">No fields selected - only name and pricing will show</p>
              )}
            </div>
          )}
          ...
          {layoutStyle === 'single' && categories.length > 0 && (
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <h3 className="text-xs uppercase tracking-wider text-white/60 font-bold">Pricing Tiers</h3>
                {loadingTiers && (
