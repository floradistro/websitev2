...
export function SmartTestimonials({
  ...
  const layoutClass = layout === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'
    : layout === 'carousel' ? 'flex overflow-x-auto gap-6 snap-x'
    : 'flex flex-col gap-6';
        ...
        {reviews.map(review => (
          ...
          >
            ...
            <div className="flex items-center gap-1 mb-3">
              {[...Array(5)].map((_, i) => (
                <span key={i} className={i < review.rating ? 'text-yellow-400' : 'text-neutral-600'}>
                  {i < review.rating ? '⭐' : '☆'}
                </span>
              ))}
            </div>
