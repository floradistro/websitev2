          ...
          <div className="relative z-10 px-4 pt-20 pb-16">
            <div className="max-w-2xl mx-auto text-center">
              <div className="inline-block mb-6 px-4 py-2 bg-emerald-500/10 border border-emerald-500/20 rounded-full">
                <span className="text-emerald-400 text-xs font-black uppercase tracking-wider">Premium Cannabis</span>
              </div>
              ...
              <div className="flex flex-col gap-4 mb-12">
                <a href={ctaPrimaryLink} className="bg-emerald-500 hover:bg-emerald-400 text-black rounded-2xl px-8 py-4 font-black uppercase tracking-tight text-sm transition-all duration-300 shadow-lg shadow-emerald-500/20">
                  {ctaPrimary}
                </a>
                <a href={ctaSecondaryLink} className="bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 text-white rounded-2xl px-8 py-4 font-black uppercase tracking-tight text-sm transition-all duration-300">
                  {ctaSecondary}
                </a>
              </div>
                ...
                <div className="flex flex-wrap justify-center gap-6 mb-12">
                  {trustBadges.map(badge => (
                    <div key={badge.id} className="flex flex-col items-center gap-2">
                      ...
                      <span className="text-white/40 text-xs uppercase font-bold tracking-wide">{badge.label}</span>
                    </div>
                  ))}
                </div>
              )}
              ...
              {showFeaturedProducts && featuredProducts.length > 0 && (
                <div className="space-y-4">
                  <h3 className="text-white/40 text-xs font-black uppercase tracking-wider mb-4">Featured Products</h3>
                  {featuredProducts.map(product => (
                    <div key={product.id} className="bg-white/5 border border-white/5 hover:border-emerald-500/20 rounded-2xl p-4 transition-all duration-300">
                      <div className="flex items-center gap-4">
                        <img src={product.image} alt={product.name} className="w-20 h-20 rounded-xl object-cover bg-white/5" />
                        <div className="flex-1 text-left">
                          <h4 className="text-white font-black uppercase text-sm mb-1">{product.name}</h4>
            ...
            <div className="max-w-7xl mx-auto grid grid-cols-2 gap-16 items-center">
              <div className="space-y-8">
                <div className="inline-block px-6 py-3 bg-emerald-500/10 border border-emerald-500/20 rounded-full">
                  <span className="text-emerald-400 text-sm font-black uppercase tracking-wider">Premium Cannabis</span>
                </div>
                ...
                <div className="flex gap-4 pt-4">
                  <a href={ctaPrimaryLink} className="bg-emerald-500 hover:bg-emerald-400 text-black rounded-2xl px-10 py-5 font-black uppercase tracking-tight text-base transition-all duration-300 shadow-2xl shadow-emerald-500/30 hover:scale-105">
                    {ctaPrimary}
                  </a>
                  <a href={ctaSecondaryLink} className="bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 text-white rounded-2xl px-10 py-5 font-black uppercase tracking-tight text-base transition-all duration-300">
                    {ctaSecondary}
                  </a>
                </div>
                  ...
                  <div className="flex gap-8 pt-8">
                    {trustBadges.map(badge => (
                      <div key={badge.id} className="flex items-center gap-3">
                        ...
                        <span className="text-white/60 text-sm uppercase font-bold tracking-wide">{badge.label}</span>
                      </div>
                    ))}
                  </div>
                )}
              ...
              {showFeaturedProducts && featuredProducts.length > 0 && (
                <div className="space-y-6">
                  <h3 className="text-white/40 text-sm font-black uppercase tracking-wider mb-6">Featured Collection</h3>
                  <div className="grid gap-6">
                    {featuredProducts.map((product, index) => (
                      <div key={product.id} className="group bg-white/5 border border-white/5 hover:border-emerald-500/30 rounded-2xl p-6 transition-all duration-500 hover:bg-white/10 hover:scale-105" style={{transitionDelay: `${index * 100}ms`}}>
                        <div className="flex items-center gap-6">
                          <div className="relative">
                            <img src={product.image} alt={product.name} className="w-28 h-28 rounded-2xl object-cover bg-white/5" />
                            <div className="absolute -top-2 -right-2 bg-emerald-500 text-black text-xs font-black uppercase px-3 py-1 rounded-full">New</div>
                          </div>
                          <div className="flex-1">
                            <h4 className="text-white font-black uppercase text-xl mb-2 group-hover:text-emerald-400 transition-colors">{product.name}</h4>
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <div className="flex items-center gap-3 text-xs text-white/40 font-bold uppercase">
                                <span className="bg-white/5 px-3 py-1 rounded-lg">{product.thc}% THC</span>
                                <span className="bg-white/5 px-3 py-1 rounded-lg">{product.cbd}% CBD</span>
                              </div>
                            </div>
                          </div>
                        </div>
