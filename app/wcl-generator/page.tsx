...
export default function WCLGeneratorPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Handle the nested component structure from the API
      const component = data.component || data;
      
      setGenerated({
        name: component.name || 'GeneratedComponent',
        wclCode: component.wclCode || '',
        tsCode: component.tsCode || '',
        wclLines: component.stats?.wclLines || component.wclCode?.split('\n').length || 0,
        tsLines: component.stats?.tsLines || component.tsCode?.split('\n').length || 0,
        reduction: component.stats?.reduction || 0
      });
