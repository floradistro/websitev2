  ...
  try {
    ...
    if (vendor?.marketing_provider === "alpineiq") {
      ...
      campaigns = alpineiqCampaigns.slice(0, limit).map((campaign: any) => ({
        ...
        status: campaign.isActive
          ...
          : campaign.archived
            ? "archived"
            : "sent",
        sent_at: campaign.scheduled
          ? new Date(campaign.scheduled * 1000).toISOString()
      ...
      campaigns = (emailCampaigns || []).map((campaign) => ({
        ...
        sent_at:
          campaign.sent_at || campaign.scheduled_for || campaign.created_at,
        stats: {
      ...
      return NextResponse.json(
        {
          ...
          message:
            "This vendor uses AlpineIQ. Please create campaigns at lab.alpineiq.com",
        },
        { status: 400 },
      );
    }
