            ...
            const promises = chunk.map(async (file: any) => {
              ...
              try {
                ...
                if (totalPixels > MAX_INPUT_PIXELS) {
                  ...
                  send({ type: 'info', fileName: file.name, message: `Resizing ${metadata.width}x${metadata.height} → ${newWidth}x${newHeight}` });
                ...
                console.log(`📊 Upscaled result: ${upscaledMetadata.width}x${upscaledMetadata.height} (${(upscaledBuffer.length / 1024 / 1024).toFixed(2)}MB)`);
                ...
                console.log(`✅ Uploaded upscaled: ${newFileName} (${upscaledMetadata.width}x${upscaledMetadata.height})`);

                // Clean temp file AFTER successful upload
                if (tempPath) {
                  await supabase.storage.from('vendor-product-images').remove([tempPath]);
                  console.log(`🗑️ Cleaned temp: ${tempPath}`);
                }
