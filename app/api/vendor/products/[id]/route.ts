...
export async function PUT(
  ...
  try {
    ...
    if (fetchError) {
      if (process.env.NODE_ENV === "development") {
        console.error(
          "❌ Database error verifying product ownership:",
          fetchError,
        );
      }
      return NextResponse.json(
    ...
    if (body.pricing_template_id || body.pricing_mode || body.pricing_tiers) {
      ...
      if (body.pricing_template_id) {
        ...
        if (template) {
          ...
          updatedPricingData.tiers = (template.default_tiers || []).map(
            (tier: any) => ({
              id: tier.id,
              label: tier.label,
        ...
        updatedPricingData.tiers = body.pricing_tiers.map(
    ...
    if (!updated) {
      if (process.env.NODE_ENV === "development") {
        console.error(
          "❌ Product update affected 0 rows - record may have been deleted",
        );
      }
      return NextResponse.json(
        {
...
export async function DELETE(
  ...
  try {
    ...
    if (fetchError) {
      if (process.env.NODE_ENV === "development") {
        console.error(
          "❌ Database error fetching product for deletion:",
          fetchError,
        );
      }
      return NextResponse.json(
