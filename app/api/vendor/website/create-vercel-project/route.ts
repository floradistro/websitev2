...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: vendor, error: vendorError } = await supabase
      .from("vendors")
      .select(
        "id, slug, store_name, github_username, github_repo_name, vercel_project_id, vercel_deployment_url",
      )
      .eq("id", vendorId)
      .single();
    ...
    if (!vendor.github_username || !vendor.github_repo_name) {
      ...
      return NextResponse.json(
        {
          error:
            "GitHub repository not configured. Please create your storefront repository first.",
        },
        { status: 400 },
      );
    }
