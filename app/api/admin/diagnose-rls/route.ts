...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log('👤 Current session:', session ? 'HAS SESSION (BAD!)' : 'No session (good)');
    ...
    console.log('🧪 Testing orders table INSERT permission...');
    ...
    console.log('✅ INSERT succeeded:', testOrder.id);
    ...
    console.log('🔁 Testing rapid inserts (race condition check)...');
    ...
    if (testIds.length > 0) {
      await supabase
        ...
        .delete()
        .in('id', testIds);
      console.log('🧹 Cleaned up', testIds.length, 'test orders');
    }
