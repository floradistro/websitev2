...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get POS transactions with tax info (exclude those linked to orders)
    ...
    orders?.forEach((order) => {
      ...
      const locName = order.locations?.[0]?.name || "No Location";
    ...
    posTransactions?.forEach((tx) => {
      ...
      const locName = tx.locations?.[0]?.name || "No Location";
