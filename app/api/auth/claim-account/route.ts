...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (customerError || !customers || customers.length === 0) {
      return NextResponse.json(
        { error: 'No account found with this email or phone number. Please contact support or create a new account.' },
        { status: 404, headers: corsHeaders }
      );
    }

    // If multiple customers found (phone number can match multiple), take the one with most loyalty points
    const customer = customers.length > 1
      ? customers.reduce((prev, current) => (current.loyalty_points || 0) > (prev.loyalty_points || 0) ? current : prev)
      : customers[0];

    if (customerError || !customer) {
      return NextResponse.json(
        { error: 'No account found with this email. Please contact support or create a new account.' },
        { status: 404, headers: corsHeaders }
      );
    }
    ...
    if (customer.auth_user_id) {
      return NextResponse.json(
        { error: 'This account is already activated. Please use "Forgot Password" if you need to reset your password.' },
        { status: 400, headers: corsHeaders }
      );
    }

    // Step 2.5: If customer doesn't have email, we need to ask for one
    if (!customer.email || customer.email.includes('@phone.local') || customer.email.includes('@alpine.local')) {
      // Customer only has phone number, need to collect email
      if (!email) {
        return NextResponse.json(
          {
            needsEmail: true,
            message: 'We found your account! Please provide an email address to complete activation.',
            customerInfo: {
    ...
    const resetRedirectUrl = redirectTo || `${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/reset-password`;
    ...
    if (resetError) {
      ...
      return NextResponse.json(
        {
          success: true,
          message: 'Account created but failed to send email. Please use "Forgot Password" to set your password.',
