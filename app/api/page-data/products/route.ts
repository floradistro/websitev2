...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (vendorPricingConfigsResult.error)
      console.warn(
        "Vendor pricing configs error:",
        vendorPricingConfigsResult.error,
      );
    ...
    (vendorPricingConfigsResult.data || []).forEach((config: any) => {
      ...
      config.blueprint.price_breaks.forEach((priceBreak: any) => {
        ...
        if (vendorPrice && vendorPrice.enabled !== false && vendorPrice.price) {
          tiers.push({
            weight:
              priceBreak.label || `${priceBreak.qty}${priceBreak.unit || ""}`,
    ...
    const locationsWithActive = (locationsResult.data || []).map(
      (loc: any) => ({
        ...loc,
