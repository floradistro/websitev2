    ...
    if (vendor) {
      try {
        ...
        const fieldsResponse = await fetch(
          `/api/vendor/products/custom-fields?vendor_id=${vendor.id}`,
        );
        ...
        {devices.length === 0 ? (
          ...
          <div className="flex gap-4 px-6 overflow-x-auto pb-4 scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-white/5 hover:scrollbar-thumb-white/30">
            {devices.map((device) => {
              ...
              // Determine actual orientation from device (fallback to landscape for devices without orientation data)
          ...
          >
            ...
            >
              ...
              <p className="text-white/60 mb-6">
                Open this URL on any device to add a new TV display
                {selectedLocation ? " at this location" : ""}:
              </p>
              ...
              <div className="mb-6">
                ...
                <div className="flex items-center gap-3">
                  <code className="flex-1 bg-black/50 border border-white/10 rounded-xl px-4 py-4 text-white/80 font-mono text-sm break-all">
                    ...
                    /tv-display?vendor_id={vendor?.id}&tv_number=
                    {Math.max(...devices.map((d) => d.tv_number), 0) + 1}
                    {selectedLocation ? `&location_id=${selectedLocation}` : ""}
                  </code>
                  <button
                    onClick={async () => {
