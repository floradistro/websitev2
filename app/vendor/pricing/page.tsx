...
export default function VendorPricingPage() {
  ...
  async function saveConfig(config: PricingConfig) {
    ...
    console.log('💰 Saving pricing config:', {
      ...
      custom_tiers: customTiers[config.id]
    });
  ...
  function toggleTierEnabled(configId: string, breakId: string) {
    console.log('🔄 TOGGLE CALLED for:', breakId);
    
    setEditingPrices(prev => {
      console.log('  - Previous state:', JSON.parse(JSON.stringify(prev)));
      
      const current = prev[configId]?.[breakId] || {};
      const currentEnabled = current.enabled === true || current.enabled === undefined;
      const newEnabled = !currentEnabled;
      
      console.log('  - Current tier state:', JSON.parse(JSON.stringify(current)));
      console.log('  - currentEnabled:', currentEnabled);
      console.log('  - newEnabled:', newEnabled);
      ...
      console.log('  - Updated state:', JSON.parse(JSON.stringify(updated)));
  ...
  // Removed unit conversion temporarily - will add back later
