...
export default function VendorBranding() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (form.logoFile) {
        ...
        fd.append("type", "logo");
        const res = await fetch("/api/supabase/vendor/upload", {
          method: "POST",
          headers: { "x-vendor-id": vendor!.id },
          body: fd,
        });
  ...
  const update = (u: Partial<BrandingFormState>) => {
    ...
    setErrors((e) => {
      const n = { ...e };
      Object.keys(u).forEach((k) => delete n[k]);
      return n;
    });
  };
              ...
              {tab === "visual" && (
                <div className="space-y-6">
                  <ImageUploader
                    ...
                    onPreviewChange={(p) => update({ logoPreview: p })}
                    aspectRatio="square"
                  />
                  <ImageUploader
                    label="Banner"
                    preview={form.bannerPreview || ""}
                    onFileChange={(f) => update({ bannerFile: f })}
                    onPreviewChange={(p) => update({ bannerPreview: p })}
                    aspectRatio="banner"
                  />
                  <FormGrid columns={2}>
                    <ColorPicker
