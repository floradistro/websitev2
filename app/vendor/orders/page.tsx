...
import { Search, Package, DollarSign, MapPin, Truck, Store, ShoppingBag, Calendar, User, ChevronRight, TrendingUp } from 'lucide-react';
...
export default function VendorOrders() {
  ...
  const getOrderTypeIcon = (type: string) => {
    switch (type.toLowerCase()) {
      case 'pickup': return <Store size={14} className="text-white/40" />;
      case 'shipping': return <Truck size={14} className="text-white/40" />;
      default: return <ShoppingBag size={14} className="text-white/40" />;
    }
  };
  ...
  return (
    <div className="w-full px-4 lg:px-0">
              ...
              <div key={locationName} className="minimal-glass p-4">
                ...
                <div className="space-y-1 text-xs">
                  ...
                  <div className="flex justify-between text-white/60">
                    ...
                    <span className="text-green-500/80">${locationStats.net_earnings.toFixed(2)}</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="minimal-glass p-12">
          <div className="text-center">
            ...
            <div className="text-white/40 text-sm">
              {search ? 'Try adjusting your search' : 'Orders will appear here once customers make purchases'}
            </div>
          </div>
        </div>
      ) : (
            ...
            {filteredOrders.map((order) => (
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-white font-medium">${order.vendorNetEarnings.toFixed(2)}</span>
                </div>
              </div>
            ))}
          </div>
            ...
            <table className="w-full">
              <thead className="border-b border-white/5 bg-black/40">
                <tr>
                  ...
                  <th className="text-right text-xs font-medium text-white/60 uppercase tracking-wider p-4">Revenue</th>
                  <th className="text-right text-xs font-medium text-white/60 uppercase tracking-wider p-4">Net</th>
              ...
              <tbody className="divide-y divide-white/5">
                {filteredOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-white/[0.02] transition-all group">
                    ...
                    <td className="p-4">
                      <span className="text-white/60 text-sm">{order.itemCount} item{order.itemCount !== 1 ? 's' : ''}</span>
                    </td>
                    <td className="p-4 text-right">
                      <span className="text-white text-sm font-medium">${order.vendorSubtotal.toFixed(2)}</span>
                    </td>
                    <td className="p-4 text-right">
                      <span className="text-green-500/80 text-sm font-medium">${order.vendorNetEarnings.toFixed(2)}</span>
                    </td>
                    <td className="p-4">
                      <Badge variant={getStatusVariant(order.status)}>{order.status}</Badge>
                    </td>
      ...
      {/* Order Detail Modal */}
      {isMounted && selectedOrderRef.current && modalOpenRef.current && createPortal(
        <OrderDetailModal
...
function OrderDetailModal({
  ...
  useEffect(() => {
    console.log('Modal mounted for order:', order.orderNumber);
    ...
    >
      ...
      >
        ...
        {order.deliveryType === 'delivery' && order.shippingAddress && (
          <div className="mb-6 p-4 bg-white/5 border border-white/5 rounded-2xl">
            <h3 className="text-xs uppercase tracking-wider text-white/60 mb-3">Shipping Information</h3>
        ...
        <div className="mb-6 p-4 bg-white/5 border border-white/5 rounded-2xl">
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between pt-2 border-t border-white/10">
              ...
              <span className="text-green-500 font-semibold text-lg">${order.vendorNetEarnings.toFixed(2)}</span>
            </div>
          </div>
        </div>
