...
export function InventoryItem({
  ...
  const stockColor = totalQuantity === 0 ? 'text-white/30' : totalQuantity <= 10 ? 'text-white/60' : 'text-white/80';

  const margin = costPrice ? ((price - costPrice) / price * 100) : null;
  const stockValue = price * totalQuantity;

  return (
    <div className={cn("rounded-2xl border overflow-hidden", ds.colors.bg.secondary, ds.colors.border.default)}>
      {/* Main Row */}
      <button
      ...
      {isExpanded && (
        <div className={cn("border-t p-4", ds.colors.border.default, "bg-black/20")}>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h4 className={cn(ds.typography.size.xs, ds.typography.transform.uppercase, ds.typography.tracking.wide, ds.colors.text.tertiary)}>
              Stock by Location
            </h4>
          </div>
