-- Fix atomic receive function to use correct column names
-- The purchase_order_items table uses 'quantity' and 'unit_price', not 'quantity_ordered' and 'unit_cost'
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- Check if receiving would exceed ordered quantity (use 'quantity' column, not 'quantity_ordered')
      ...
      -- Use 'unit_price' column, not 'unit_cost'
...
COMMENT ON FUNCTION receive_purchase_order_items IS 'Atomic function to receive purchase order items. Handles inventory updates, receiving records, and stock movements in a single transaction. Fixed to use correct column names: quantity (not quantity_ordered) and unit_price (not unit_cost).';
