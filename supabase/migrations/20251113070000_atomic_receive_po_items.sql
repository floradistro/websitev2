-- Create atomic function to receive purchase order items
-- This function handles all the receiving logic in a single transaction
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      IF (COALESCE(v_po_item.quantity_received, 0) + v_quantity_received) > v_po_item.quantity_ordered THEN
        v_results := v_results || jsonb_build_object(
          'success', false,
          'item_id', v_item->>'po_item_id',
...
COMMENT ON FUNCTION receive_purchase_order_items IS 'Atomic function to receive purchase order items. Handles inventory updates, receiving records, and stock movements in a single transaction.';
