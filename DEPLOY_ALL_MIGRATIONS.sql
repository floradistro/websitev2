...
-- =====================================================
-- MIGRATION 1: Atomic Product Creation
-- =====================================================
...
DECLARE
  ...
  v_variant_ids UUID[];
BEGIN
  -- Validate primary location exists
  ...
  v_product_type := p_product_data->>'product_type';
...
'Atomically creates a product with inventory and variants. All-or-nothing transaction.';
...
SELECT proname,
       ...
       CASE WHEN prosrc LIKE '%FOR UPDATE NOWAIT%' THEN 'YES' ELSE 'NO' END as has_locking
FROM pg_proc
WHERE proname IN (
