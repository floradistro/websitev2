...
exports.autoFixDesign = autoFixDesign;
...
function validateStorefront(design, vendorData) {
    const errors = [];
    const warnings = [];
    ...
    const usesVendorName = design.components.some(c => c.component_key === 'text' &&
        c.props.text &&
    ...
    const hasRandomSpacing = spacerHeights.some(h => ![8, 12, 16, 20, 24, 32, 40, 48, 60, 80, 100].includes(h));
    if (hasRandomSpacing) {
    ...
    const hasNonCenteredText = textComponents.some(c => c.props.alignment && c.props.alignment !== 'center');
...
function autoFixDesign(design, validationResult) {
    let fixed = { ...design };
