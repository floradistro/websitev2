...
export function validateStorefront(
  ...
  const hasHeader =
    design.sections.some((s) => s.section_key === "header") ||
  ...
  const hasFooter =
    design.sections.some((s) => s.section_key === "footer") ||
  ...
  if (design.sections.length < 4) {
    errors.push(
      "Storefront must have at least 4 sections (header, hero, content, footer)",
    );
  }
  ...
  if (!hasSmartComponent) {
    errors.push(
      "Storefront must have at least one smart component (products/locations/reviews)",
    );
  }
  ...
  design.components.forEach((comp, idx) => {
    if (!VALID_COMPONENTS.includes(comp.component_key)) {
      errors.push(
        `Invalid component type at position ${idx}: ${comp.component_key}`,
      );
    }
  });
  ...
  design.sections.forEach((section, idx) => {
    if (!VALID_SECTIONS.includes(section.section_key)) {
      errors.push(
        `Invalid section type at position ${idx}: ${section.section_key}`,
      );
    }
  });
  ...
  design.components.forEach((comp, idx) => {
    if (comp.component_key === "text" && comp.props.text) {
      ...
      if (
        ...
        errors.push(
          `Component ${idx} has placeholder text: "${comp.props.text}"`,
        );
      }
  ...
  const hasProductGrid = design.components.some(
    (c) => c.component_key === "smart_product_grid",
  );
  if (hasProductGrid && vendorData.product_count === 0) {
    warnings.push(
      "Vendor has 0 products but storefront includes product grid. Consider removing or adding sample products.",
    );
  }

  // Rule 8: Verify vendor has locations if using smart_locations
  const hasLocations = design.components.some(
    (c) => c.component_key === "smart_locations",
  );
  if (hasLocations && (vendorData.location_count || 0) === 0) {
    warnings.push(
      "Vendor has 0 locations but storefront includes location map. Consider removing.",
    );
  }
  ...
  design.components.forEach((comp, idx) => {
    if (!sectionKeys.has(comp.section_key)) {
      errors.push(
        `Component ${idx} references non-existent section: ${comp.section_key}`,
      );
    }
  });
  ...
  const usesVendorName = design.components.some(
    (c) =>
  ...
  const hasRandomSpacing = spacerHeights.some(
    (h) => ![8, 12, 16, 20, 24, 32, 40, 48, 60, 80, 100].includes(h),
  );
  if (hasRandomSpacing) {
    warnings.push(
      "Inconsistent spacing detected. Use 8, 12, 16, 24, 32, 40, 48, 60, 80",
    );
  }

  // Rule 13: Check for color consistency (not using random colors)
  const textComponents = design.components.filter(
    (c) => c.component_key === "text",
  );
  const colors = textComponents.map((c) => c.props.color).filter(Boolean);

  const uniqueColors = [...new Set(colors)];
  if (uniqueColors.length > 5) {
    warnings.push(
      `Too many colors used (${uniqueColors.length}). Stick to 2-3 brand colors max.`,
    );
  }

  // Rule 14: Check for center alignment
  const hasNonCenteredText = textComponents.some(
    (c) => c.props.alignment && c.props.alignment !== "center",
  );
  if (hasNonCenteredText) {
    warnings.push(
      "Some text is not center-aligned. Center alignment provides cleaner look.",
    );
  }
...
export function autoFixDesign(
  ...
  const hasHeaderSection = fixed.sections.some(
    (s) => s.section_key === "header",
  );
  if (!hasHeaderSection) {
  ...
  if (!fixed.sections.find((s) => s.section_key === "hero")) {
    const firstContentIdx = fixed.sections.findIndex(
      (s) => s.section_order >= 0,
    );
  ...
  const hasFooterSection = fixed.sections.some(
  ...
  fixed.sections = fixed.sections
    ...
    .map((s, idx) => ({
      ...s,
      section_order:
        s.section_key === "header"
          ? -1
          : s.section_key === "footer"
            ? 999
            : idx,
    }));
