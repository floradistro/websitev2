...
export function validateStorefront(
  ...
  const hasSmartComponent = design.components.some((c) => c.component_key.startsWith("smart_"));
  ...
  const hasProductGrid = design.components.some((c) => c.component_key === "smart_product_grid");
  ...
  const hasLocations = design.components.some((c) => c.component_key === "smart_locations");
  ...
  const usesVendorName = design.components.some(
    (c) =>
      c.component_key === "text" && c.props.text && c.props.text.includes(vendorData.store_name),
  );
  if (!usesVendorName) {
...
export function autoFixDesign(
  ...
  fixed.sections = fixed.sections
    ...
    .map((s, idx) => ({
      ...s,
      section_order: s.section_key === "header" ? -1 : s.section_key === "footer" ? 999 : idx,
    }));

  // Auto-fix: Remove components for non-existent sections
  const validSectionKeys = new Set(fixed.sections.map((s) => s.section_key));
  fixed.components = fixed.components.filter((c) => validSectionKeys.has(c.section_key));
