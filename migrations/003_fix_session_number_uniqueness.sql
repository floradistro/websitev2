-- Fix session number generation to guarantee uniqueness
-- Add milliseconds + random suffix to prevent collisions
...
BEGIN
  ...
  -- Generate unique session number with millisecond precision + random suffix
  v_timestamp_ms := EXTRACT(EPOCH FROM clock_timestamp() AT TIME ZONE 'UTC') * 1000;
  v_session_number := 'S-' || v_timestamp_ms::TEXT || '-' || LPAD(FLOOR(RANDOM() * 10000)::TEXT, 4, '0');
