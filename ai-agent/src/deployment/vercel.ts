...
export class VercelDeployment {
  ...
  async deploy(config: DeploymentConfig): Promise<DeploymentResult> {
    logger.debug(`🔵 Deploying ${config.vendorSlug} to Vercel...`);
    ...
    try {
      ...
      logger.debug(`✅ Deployed to ${deployment.deploymentUrl}`);
  ...
  private async configureDomain(projectId: string, domain: string): Promise<void> {
    logger.debug(`🔵 Configuring custom domain: ${domain}`);
    ...
    logger.debug(`✅ Domain configured: ${domain}`);
  }
  ...
  async deleteDeployment(deploymentId: string): Promise<void> {
    ...
    logger.debug(`✅ Deployment deleted: ${deploymentId}`);
  }
}
